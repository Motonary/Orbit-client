...
const HeaderRightLinks: React.FC<Props> = ({ currentUser, history, pathname }) => {
  ...
  const isBackButtonShow = pathname === `${rootPath}/history` || pathname === `${rootPath}/edit`

  return (
    <Root>
      <_Link onClick={() => onLinkButtonClick(`${rootPath}/history`)} isShow={isHistoryButtonShow}>
        HISTORY
        <Icon src={HeaderIcons['HISTORY']} alt="history" height="50px" width="50px" />
      </_Link>
      <_Link onClick={() => onLinkButtonClick(`${rootPath}/edit`)} isShow={isSettingButtonShow}>
        SETTING
        <Icon src={HeaderIcons['SETTING']} alt="setting" height="50px" width="50px" />
      </_Link>
